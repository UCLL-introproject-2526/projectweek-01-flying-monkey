...
def speel(SCREEN, pause_func=None, assets=None, sfx_on=True):
    ...
    # Zorg dat de onderkant van het kasteel gelijk is aan de grond (HEIGHT - 50)
    castle_height = 150
    ground_y = HEIGHT - 50
    castle = Castle(castle_x, ground_y - castle_height, 120, castle_height, assets, sfx_on)
    ...
    while running:
        ...
        if not (win or game_over):
            ...
            # Coin collection
            for plat in platforms:
                pass # coins are not in level2, skip
        ...
        if not game_over:
            ...
            # Win text pas na 4 seconden animatie
            if win:
                if not hasattr(castle, 'win_time'):
                    castle.win_time = pygame.time.get_ticks()
                time_since_win = (pygame.time.get_ticks() - castle.win_time) / 1000.0
                if time_since_win >= 4.0:
