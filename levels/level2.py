...
def speel(SCREEN, pause_func=None):
    ...
    BG_IMG = ASSETS["bg2"] if ASSETS["bg2"] else ASSETS["bg"]
    ...
    # VARIABELEN
    player = pygame.Rect(100, HEIGHT - 150, 40, 50)
    player_vel_y = 0
    speed = 5
    jump_power = -15
    gravity = 1
    ...
    flag_rect = pygame.Rect(castle_x - 50, HEIGHT - 250, 20, 100)
    ...
    while running:
        ...
        if not game_over:
            ...
            for e in enemies:
                if ASSETS["knife"]: SCREEN.blit(ASSETS["knife"], (e.x - 10 - camera_x, e.y - 10))
                else: pygame.draw.rect(SCREEN, RED, (e.x - camera_x, e.y, e.width, e.height))
